<section>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        <div>
            ...
            <x-text-input id="name" name="name" type="text" class="form-control-full" :value="old('name', $user->name)" required autofocus autocomplete="name" />
            <x-input-error class="mt-2" :messages="$errors->get('name')" />
        </div>

        <div>
            <x-input-label for="email" :value="__('Correo electronico')" />
            <x-text-input id="email" name="email" type="email" class="form-control-full" :value="old('email', $user->email)" required autocomplete="username" />
            ...
            @if ($user instanceof \Illuminate\Contracts\Auth\MustVerifyEmail && ! $user->hasVerifiedEmail())
                <div>
                    ...
                    @if (session('status') === 'verification-link-sent')
                        <p class="mt-2 form-status-success">
        ...
        <div>
            ...
            <div class="form-phone-row">
                ...
                <x-text-input id="phone_number" name="phone_number" type="tel" class="form-control-fill" :value="old('phone_number', $phoneParts['phone_number'])" required autocomplete="tel-national" placeholder="612345678" />
            </div>
